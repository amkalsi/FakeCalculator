...

{
	...
	sftool_loose = new TauIDSFTool(LooseIsoWPFile, 2017, "MVAoldDM2017v2" , "VLoose", false);
	sftool_tight = new TauIDSFTool(LooseIsoWPFile,2017,"MVAoldDM2017v2","Tight",false);
...
LowMTRegion_2017::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	{     
		...
		if(!isdata &&  ( DYSample.find("TT") != npos || DYSample.find("TTinc") != npos )) {
			...
			/*for(unsigned iMC=0 ; iMC<tree->mc_pdgId->at(iMC) ; ++iMC){
			  TLorentzVector e1;
			  TLorentzVector t1;
			  bool ele1_found=false;
			  bool tau1_found=false;

			  if( fabs(tree->mc_pdgId->at(iMC)) == 11 && fabs(tree->mc_status->at(iMC)) == 23) {
			  e1.SetPxPyPzE(tree->mc_px->at(iMC), tree->mc_py->at(iMC),tree->mc_pz->at(iMC), tree->mc_energy->at(iMC)); ele1_found=true;break; ;}

			  if( fabs(tree->mc_pdgId->at(iMC)) == 15 && fabs(tree->mc_status->at(iMC)) == 23) { 
			  t1.SetPxPyPzE(tree->mc_px->at(iMC), tree->mc_py->at(iMC),tree->mc_pz->at(iMC), tree->mc_energy->at(iMC)); tau1_found=true;break; ;}



			  }
			  if(ele1_found && tau1_found) { // put mass here;
			  }
			  */
		}
		...
		for(unsigned int imuon=0; imuon < tree->mu_ibt_pt->size(); imuon++) {
			...
			for (unsigned int dau1index = 0; dau1index < ele_indexes.size(); dau1index++){
				...
				TLorentzVector DauEle ;
				DauEle.SetPtEtaPhiM(tree->gsf_pt->at(ele_indexes.at(dau1index)), tree->gsf_eta->at(ele_indexes.at(dau1index)), tree->gsf_phi->at(ele_indexes.at(dau1index)),m_el);
...
float LowMTRegion_2017::GetEfficiency(float eta, float pt, TH2F *hist) 
{
...
double LowMTRegion_2017::MutoTauFR(TLorentzVector tau) {
	int Otype=0;
	double DR=0.2;
	double SF_fake =1.;
	double SF_fake_1 =1.;
	...
	if(Otype == 1 || Otype == 3) {
		...
		if( fabs(tau.Eta())  < 1.460) { SF_fake = 1.17;}//1.96; }
	if( fabs(tau.Eta())  > 1.558) { SF_fake = 1.25;}//1.66; }
