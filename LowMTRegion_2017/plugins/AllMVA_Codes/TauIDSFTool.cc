...
TauIDSFTool::TauIDSFTool(const std::string& filename, const int year, const std::string& id, const std::string& wp, const bool dm): File(filename), ID(id), WP(wp){
...
//  bool verbose = false;
  ...
  if(std::find(antiJetIDs.begin(),antiJetIDs.end(),ID)!=antiJetIDs.end()){
    if(dm){
      TFile* file = TFile::Open(TString(File),"READ");//"%s/TauID_SF_dm_%s_%d.root",datapath.data(),ID.data(),year);
      ...
      TFile* file  = TFile::Open(TString(File),"READ");
