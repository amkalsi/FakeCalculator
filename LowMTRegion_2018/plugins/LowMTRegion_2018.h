// -*- C++ -*-
//
// Package:    Plots/LowMTRegion_2018
// Class:      LowMTRegion_2018
// 
/**\class LowMTRegion_2018 LowMTRegion_2018.cc Plots/LowMTRegion_2018/plugins/LowMTRegion_2018.cc
...
class LowMTRegion_2018 : public edm::EDAnalyzer {
	public:
		explicit LowMTRegion_2018(const edm::ParameterSet&);
		~LowMTRegion_2018();
	...
	private:
		...
		float GetEfficiency(float eta, float pt) ;
		TH2F *fhDMuMediumSF ; TH2F *fhDMuMediumSF_endcap;
		TH2F *fhDMuIsoSF;
		...
		double MutoTauFR(TLorentzVector tau, string, string);
		...
		double TauPtBinSF_CR5(double taupt, double abs_taueta, int decaymode) ;
		...
		static const int fakerate_num_N = 20;
		...
		TH1F *h7[4][4][2];
		TH1F *h8[4][4][2];
		TH1D *h_Fill_RawEvents;
		TH1D *h_Fill_WeightedEvents;
		TH1D *h_Events_After_Trigger;
		TH1D *h_Fill_TauPass;
		TH1D *h_Fill_ElePass;
		TH1D *h_Events_After_MetFilters;
		TH1D *h_Fill_MuonExtra;
		TGraphAsymmErrors*  eTrigger_barrel;
		TGraphAsymmErrors*  eTrigger_endcap;
		bool HEEPwoTkIso_choice2(unsigned int eleindex);
		TauIDSFTool *sftool_loose;
		TauIDSFTool *sftool_tight;
		string LooseIsoWPFile, TightIsoWPFile;
		double GetTTWeight(double ttmass);
		double event_wt_NNPDF ;
		TH1F *histo_LowPtTES;
		TH1F *histo_HighPtTES;
		string LowPtTES, HighPtTES;
		string FESDM ;
		TGraph *graph_FESDM;
		double getTESSF(double taupt, int taudecaymode, string type_of);
		double GetTopQscale(TString process, float Mll, TString var);
		double GetTopPDF(TString process, float Mll, TString var);

		double  getEFakeTESSF(double taupt, int taudecaymode, string type_of);
		double GetWWNLO(TString process, float Mll, TString var);
		TLorentzVector ElectronTauFakeEnergyScale(TLorentzVector tau, int taudecaymode, string type_of);
		TGraph *graph_FES;
		TLorentzVector TauEnergyScale(TLorentzVector tau, int taudecaymode, string type_of);
