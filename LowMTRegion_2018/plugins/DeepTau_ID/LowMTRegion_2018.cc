...

{
	...
	//	sftool_loose = new TauIDSFTool(LooseIsoWPFile, 2018, "MVAoldDM2017v2" , "VVVLoose", false);
	//	sftool_tight = new TauIDSFTool(LooseIsoWPFile,2018,"MVAoldDM2017v2","Medium",false);

	sftool_loose = new TauIDSFTool(LooseIsoWPFile, 2018, "DeepTau2017v2p1" , "VVVLoose", false);
	sftool_tight = new TauIDSFTool(LooseIsoWPFile,2018,"DeepTau2017v2p1","Medium",false);
...
LowMTRegion_2018::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	{     
		...
		for (unsigned int dau2index = 0; dau2index < tree->tau_pt->size(); dau2index++){
			...
			if(!(tree->tau_byLooseDeepTau2017v2p1VSmu->at(dau2index) > 0.5)) continue;
			// HEM
		...
		for (unsigned int dau1index = 0; dau1index < tree->gsf_caloEnergy->size(); dau1index++){
			...
			for (unsigned int dau2index = 0; dau2index < tree->tau_pt->size(); dau2index++){
				...
				if(tree->gsf_charge->at(dau1index) * tree->tau_charge->at(dau2index) == -1 ) {
					if (tree->tau_byMediumDeepTau2017v2p1VSjet->at(dau2index) > 0.5) {
						int hadtau=0;
					...
					if ((tree->tau_byMediumDeepTau2017v2p1VSjet->at(dau2index) < 0.5) && (tree->tau_byVVVLooseDeepTau2017v2p1VSjet->at(dau2index) > 0.5)) {
...
double LowMTRegion_2018::MutoTauFR(TLorentzVector tau) {
	...
	if(Otype == 1 || Otype == 3) {
		...
		if( fabs(tau.Eta()) > 0 && fabs(tau.Eta()) < 0.4) { SF_fake = 0.929448;}// 1.05;}//\pm0.05 ; }
		if( fabs(tau.Eta()) > 0.4 && fabs(tau.Eta()) < 0.8) { SF_fake = 1.19825;}//0.96;}//\pm0.04 ; }
		if( fabs(tau.Eta()) > 0.8 && fabs(tau.Eta()) < 1.2) { SF_fake = 1.06248;}//1.06; }//\pm0.05 } //1.10; }
		if( fabs(tau.Eta()) > 1.2 && fabs(tau.Eta()) < 1.7) { SF_fake = 0.81958;}//1.45;}//\pm0.08 1.03; }
		if( fabs(tau.Eta()) > 1.7 && fabs(tau.Eta()) < 2.3) { SF_fake = 6.20662;}//1.75;}//\pm0.16 1.94; }



		}  else if ( Otype == 2 || Otype == 4) {

			if( fabs(tau.Eta())  < 1.460) { SF_fake = 1.06;}//1.229;}//\pm0.018; }
			if( fabs(tau.Eta())  > 1.558) { SF_fake = 0.78;}//0.926;}//\pm0.015; 1.66; }
