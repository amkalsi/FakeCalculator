...
//	eTrigger_endcap = (TGraphAsymmErrors*) (fMuSF->Get("endcap"));
 fhDMuMediumSF = (TH2F*)(fMuSF->Get("SF_TH2F_Barrel"));
	...
	sftool_loose = new TauIDSFTool(LooseIsoWPFile, 2018, "DeepTau2017v2p1" , "VVVLoose", false);
	sftool_tight = new TauIDSFTool(LooseIsoWPFile,2018,"DeepTau2017v2p1","Tight",false);
...
//		}
		...
		/*
		   ...
		   if( DYSample.find("DY_") != npos ||  DYSample.find("WW_") != npos || DYSample.find("TT_") != npos || DYSample.find("WWinc")  != npos) {
		   pu_weight =PU_2017::MC_pileup_weight(tree->mc_trueNumInteractions, DYSample, "Data_for2017"); 
		   } else {pu_weight =PU_2018_Rereco::MC_pileup_weight(tree->mc_trueNumInteractions, "MC", "Data_2018AtoD"); } 

		   }
		   */
		if(!isdata ) {pu_weight =PU_2018_Rereco::MC_pileup_weight(tree->mc_trueNumInteractions, "MC", "Data_2018AtoD"); } 
		else {pu_weight = 1;}

		if(DYSample.find("WW_") != npos || DYSample.find("TT_") != npos) {
			pu_weight = PU_2017_Rereco::MC_pileup_weight(tree->mc_trueNumInteractions, DYSample, "Rereco_all");
		}
		...
		for (unsigned int dau1index = 0; dau1index < tree->gsf_caloEnergy->size(); dau1index++){
			...
			for (unsigned int dau2index = 0; dau2index < tree->tau_pt->size(); dau2index++){
				...
				double fake_wt = 1.;
				double sf_heep= 1.;
				if( (!isdata) )  {  if( fabs(tree->gsf_sc_eta->at(dau1index)) < 1.4442) {weight_sd = GetEfficiency(tree->gsf_sc_eta->at(dau1index), DauEle.Pt()); } else { weight_sd =  GetEfficiency(tree->gsf_sc_eta->at(dau1index), DauEle.Pt()); }}
				...
				if(tree->gsf_charge->at(dau1index) * tree->tau_charge->at(dau2index) == -1 ) {
					if (tree->tau_byTightDeepTau2017v2p1VSjet->at(dau2index) > 0.5) {
						...
						h7[0][j_dm][k_eta]->Fill(tree->tau_eta->at(dau2index), final_weight*sftool_tight->getSFvsPT(tree->tau_pt->at(dau2index),hadtau));

						h6[0][j_dm][k_eta]->Fill(tree->pv_n, final_weight*sftool_tight->getSFvsPT(tree->tau_pt->at(dau2index),hadtau));
					...
					if ((tree->tau_byTightDeepTau2017v2p1VSjet->at(dau2index) < 0.5) && (tree->tau_byVVVLooseDeepTau2017v2p1VSjet->at(dau2index) > 0.5)) {
						...
						h7[1][j_dm][k_eta]->Fill(tree->tau_eta->at(dau2index), final_weight*sftool_loose->getSFvsPT(tree->tau_pt->at(dau2index),hadtau));

						h6[1][j_dm][k_eta]->Fill(tree->pv_n, final_weight*sftool_loose->getSFvsPT(tree->tau_pt->at(dau2index),hadtau));
					...
					if (tree->tau_byTightDeepTau2017v2p1VSjet->at(dau2index) > 0.5) {
						...
						h7[2][j_dm][k_eta]->Fill(tree->tau_eta->at(dau2index), final_weight*sftool_tight->getSFvsPT(tree->tau_pt->at(dau2index),hadtau_yx));

						h6[2][j_dm][k_eta]->Fill(tree->pv_n, final_weight*sftool_tight->getSFvsPT(tree->tau_pt->at(dau2index),hadtau_yx));
					...
					if ((tree->tau_byTightDeepTau2017v2p1VSjet->at(dau2index) < 0.5) && (tree->tau_byVVVLooseDeepTau2017v2p1VSjet->at(dau2index) > 0.5)) {
						...
						h7[3][j_dm][k_eta]->Fill(tree->tau_eta->at(dau2index), final_weight*sftool_loose->getSFvsPT(tree->tau_pt->at(dau2index),hadtau_yx));

						h6[3][j_dm][k_eta]->Fill(tree->pv_n, final_weight*sftool_loose->getSFvsPT(tree->tau_pt->at(dau2index),hadtau_yx));
	...
	for (unsigned int i = 0; i< numH_name; ++i) {
		for (unsigned int k = 0; k< numH_dms ; ++k) {
			for (unsigned int l = 0; l< num_etaarr; ++l) {
				...
				TString nname6 = h_names[i]+"_"+dms[k]+"_"+etaarr[l]+"_nvtx";
				TString nname7 = h_names[i]+"_"+dms[k]+"_"+etaarr[l]+"_eta";
				...
				h7[i][k][l] = fs->make<TH1F>(nname7, nname7, 100, -3,3);
				h6[i][k][l] = fs->make<TH1F>(nname6, nname6, 150, 0, 150);
...
float LowMTRegion_2018::GetEfficiency(float eta, float pt) 
{
...
TLorentzVector LowMTRegion_2018::TauEnergyScale(TLorentzVector tau, int taudecaymode, string type_of){
...
double LowMTRegion_2018::getTESSF(double taupt, int taudecaymode, string type_of){
...
TLorentzVector LowMTRegion_2018::ElectronTauFakeEnergyScale(TLorentzVector tau, int taudecaymode, string type_of){
...
double LowMTRegion_2018::getEFakeTESSF(double taueta, int taudecaymode, string type_of){
	double sf_etauFS = 1;
	int i=-1;
...
double LowMTRegion_2018::GetTopQscale(TString process, float Mll, TString var){
	if (Mll <0 || var=="nom") {
		return 1;
	}
	else {
...
double LowMTRegion_2018::GetTopPDF(TString process, float Mll, TString var){
	if (Mll <0 || var=="nom") {
		return 1;
	}
	else {
...
double LowMTRegion_2018::GetWWNLO(TString process, float Mll, TString var){
	if (Mll <0 || var=="nom") {
		return 1;
	}
	else {
...
double LowMTRegion_2018::MutoTauFR(TLorentzVector tau,string type_unc, string apply_to_what) {
	int Otype=0;
	double DR=0.2;
	double SF_fake =1.;
	for (unsigned int iGen = 0; iGen < tree->mc_px->size(); iGen++){
	...
	if(Otype == 1 || Otype == 3) {
		...
		double sf_r_1 = 0.76752; double err_r_1 = 0.126023;
		double sf_r_2 = 1.25506; double err_r_2 = 0.258356;

		double sf_r_3 = 0.901968; double err_r_3 = 0.202822;
		double sf_r_4 = 0.833875 ; double err_r_4 = 0.41508;
		double sf_r_5 = 4.4357 ; double err_r_5 = 0.8139;
		if( fabs(tau.Eta()) > 0 && fabs(tau.Eta()) < 0.4) { if(type_unc == "normal") {SF_fake = sf_r_1;} else if(type_unc == "up" && apply_to_what == "mutau") {SF_fake= sf_r_1+err_r_1;} else if(type_unc == "down" && apply_to_what == "mutau") { SF_fake = sf_r_1-err_r_1;}  } 
		...
		double sf_e_1 = 1.06; double err_e_1 = 0.09;
		double sf_e_2 =0.78; double err_e_2 = 0.12;
